...
<head>
    ...
    <title>Daftar - CareerSense</title>
...
<main class="auth-shell">
    <section class="auth-brand">
        ...
        <div>
            <span class="text-[11px] font-black uppercase tracking-[.22em]" style="color:#00d4ff;">Start Your Workspace</span>
            <h1>Buat akun CareerSense untuk membuka potensi CV.</h1>
            <p class="max-w-xl text-slate-300 leading-7">Daftar manual atau langsung gunakan Google seperti alur SkripsiCare, tetap dengan tema gelap neon CareerSense.</p>
        </div>
        <div class="grid gap-3 text-sm text-cyan-100">
            <span>✓ Workspace user dan admin terpisah berdasarkan role</span>
            <span>✓ Subscription mengatur akses dashboard</span>
        </div>
    </section>
    <section class="auth-card">
        <div class="auth-form">
            <div class="mb-7 text-center">
                ...
                <h2 class="mt-4 text-3xl font-black">Daftar</h2>
                <p class="mt-1 text-sm text-slate-400">Buat akun CareerSense baru.</p>
            </div>
            <form action="{{ route('register.submit') }}" method="POST" class="grid gap-4">
                @csrf
            ...
            <p class="mt-6 text-center text-sm text-slate-500">Sudah punya akun? <a href="{{ route('login') }}" class="font-bold text-cyan-400">Masuk</a></p>
        </div>
    </section>
</main>
@if($errors->any())
